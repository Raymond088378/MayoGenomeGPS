...
### tool to convert segseq output to vcf output 
    ##  contact : Saurabh Baheti
    ##	email	: baheti.saurabh@mayo.edu
    ##	date	: work in progress

##  deletion
#chr6    163071341       163071530       s_MCA007_T      0.1803  +


##  duplication
#chr6    44149047        44150108        s_MCA007_T      1.7348  +
...
my($input, $output, $ref, $samtools, $sample, $help);
GetOptions("in|i=s"     => \$input,
        "out|o:s"       => \$output,
        "ref|r:s"     => \$ref,
        "samtool|t:s"   => \$samtools,
        "sample|s:s"    => \$sample,
        "help|h|?|"     => \&help);
...
open IN, "<$input" or die "can not read Segseq input file $input :$! \n";
open OUT, ">$output" or die "can not write VCF file $output :$! \n";
open FAIL, ">${output}.fail" or die " can not write fail calls :$!\n";
...
sub help{
		print "DESCRIPTION:
		SegSeq2vcf.pl scripts convert segseq output to VCF output
		USAGE:
		SegSeq2vcf.pl -i input_file -r reference file -o output.vcf -s sample
		
		OPTIONS:
		
		--in,-i   		input file from segseq tool
		--out,-o		output vcf file
		--sample,-s		sample name to put in vcf file
        --samtools,-t   path to samtools
		--ref,-r		reference file used to run segseq	
