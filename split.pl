...
sub pph {
	...
	`mkdir -p $output.$num.poly`;
	`/usr/local/biotools/perl/5.10.0/bin/perl $poly/bin/run_pph.pl -A -g $genome -d $output.$num.poly $input.$num > $output 2> $log`;
	`rm $log`;
	`rm $input.$num`;
	`rm -R $output.$num.poly`;
}
