...
#################################################################################################					### generating gene summary file
		...
		# for group in `echo $groups | tr ":" " "`
				# do
					...
					# do
						...
						# then
							...
							##############################################################################	## summarizing SNV files
							# cat $SNV_dir/$sample.SNV.cleaned_annot.xls | cut -f46,50 > $SNV_dir/$sample.SNV.tmp
							# cat $SNV_dir/$sample.SNV.tmp | grep nonsense | cut -f2 | tr "," "\n" > $SNV_dir/$sample.nonsense.tmp
							# cat $SNV_dir/$sample.SNV.tmp | grep missense | cut -f2 | tr "," "\n" > $SNV_dir/$sample.missense.tmp
							# cat $SNV_dir/$sample.SNV.tmp | grep coding-synonymous | cut -f2 | tr "," "\n" > $SNV_dir/$sample.codingsynonymous.tmp
							# cat $SNV_dir/$sample.SNV.tmp | grep coding-notMod3 | cut -f2 | tr "," "\n" > $SNV_dir/$sample.codingnotMod3.tmp
							...
							# Rscript $script_path/summary.SNV.r $report_dir/gene.temp $SNV_dir/$sample.nonsense.tmp $SNV_dir/$sample.missense.tmp $SNV_dir/$sample.codingsynonymous.tmp $SNV_dir/$sample.codingnotMod3.tmp $SNV_dir/$sample.splice3.tmp $SNV_dir/$sample.splice5.tmp $SNV_dir/$sample.utr3.tmp $SNV_dir/$sample.utr5.tmp $SNV_dir/$sample.nonsense.txt $SNV_dir/$sample.missense.txt $SNV_dir/$sample.codingsynonymous.txt $SNV_dir/$sample.codingnotMod3.txt $SNV_dir/$sample.splice3.txt $SNV_dir/$sample.splice5.txt $SNV_dir/$sample.utr3.txt $SNV_dir/$sample.utr5.txt
							
							# join $SNV_dir/$sample.nonsense.txt $SNV_dir/$sample.missense.txt > $SNV_dir/join1.txt
							# join $SNV_dir/join1.txt $SNV_dir/$sample.codingsynonymous.txt > $SNV_dir/join2.txt
							# join $SNV_dir/join2.txt $SNV_dir/$sample.codingnotMod3.txt > $SNV_dir/join3.txt
							# join $SNV_dir/join3.txt $SNV_dir/$sample.splice3.txt > $SNV_dir/join4.txt
							# join $SNV_dir/join4.txt $SNV_dir/$sample.splice5.txt > $SNV_dir/join5.txt
							# join $SNV_dir/join5.txt $SNV_dir/$sample.utr3.txt > $SNV_dir/join6.txt
							# join $SNV_dir/join6.txt $SNV_dir/$sample.utr5.txt > $SNV_dir/join7.txt
							# cat $SNV_dir/join7.txt | tr " " "\t" > $SNV_dir/join8.txt
							
							# touch $SNV_dir/$sample.SNV.summary
							# echo -e "NONSENSE\tMISSENSE\tCODING-SYNONYMOUS\tCODING-NOTMOD3\tSPLICE-3\tSPLICE-5\tUTR-3\tUTR-5" >> $SNV_dir/$sample.SNV.summary
							# cat $SNV_dir/join8.txt >> $SNV_dir/$sample.SNV.summary
							# Rscript $script_path/sum.cols.r $SNV_dir/$sample.SNV.summary $SNV_dir/$sample.SNV.sum
							
							# rm $SNV_dir/*.tmp $SNV_dir/join*.txt $SNV_dir/$sample.nonsense.txt $SNV_dir/$sample.missense.txt $SNV_dir/$sample.codingsynonymous.txt $SNV_dir/$sample.codingnotMod3.txt $SNV_dir/$sample.splice3.txt $SNV_dir/$sample.splice5.txt $SNV_dir/$sample.utr3.txt $SNV_dir/$sample.utr5.txt
					######################################################################################	
							## summarizing INDEL files
							# cat $INDEL_dir/$sample.INDEL.cleaned_annot.xls | cut -f16,20 > $INDEL_dir/$sample.INDEL.tmp
							# cat $INDEL_dir/$sample.INDEL.tmp | grep coding | cut -f2 | tr "," "\n" > $INDEL_dir/$sample.coding.tmp
							# cat $INDEL_dir/$sample.INDEL.tmp | grep frameshift | cut -f2 | tr "," "\n" > $INDEL_dir/$sample.frameshift.tmp
							# cat $INDEL_dir/$sample.INDEL.tmp | grep splice-3 | cut -f2 | tr "," "\n" > $INDEL_dir/$sample.splice3.tmp
							# cat $INDEL_dir/$sample.INDEL.tmp | grep splice-5 | cut -f2 | tr "," "\n" > $INDEL_dir/$sample.splice5.tmp
							...
							# Rscript $script_path/summary.INDEL.r $report_dir/gene.temp $INDEL_dir/$sample.coding.tmp $INDEL_dir/$sample.frameshift.tmp $INDEL_dir/$sample.splice3.tmp $INDEL_dir/$sample.splice5.tmp $INDEL_dir/$sample.utr3.tmp $INDEL_dir/$sample.utr5.tmp $INDEL_dir/$sample.coding.txt $INDEL_dir/$sample.frameshift.txt $INDEL_dir/$sample.splice3.txt $INDEL_dir/$sample.splice5.txt $INDEL_dir/$sample.utr3.txt $INDEL_dir/$sample.utr5.txt
							
							# join $INDEL_dir/$sample.coding.txt $INDEL_dir/$sample.frameshift.txt > $INDEL_dir/join1.txt
							# join $INDEL_dir/join1.txt $INDEL_dir/$sample.splice3.txt > $INDEL_dir/join2.txt
							# join $INDEL_dir/join2.txt $INDEL_dir/$sample.splice5.txt > $INDEL_dir/join3.txt
							# join $INDEL_dir/join3.txt $INDEL_dir/$sample.utr3.txt > $INDEL_dir/join4.txt
							# join $INDEL_dir/join4.txt $INDEL_dir/$sample.utr5.txt > $INDEL_dir/join5.txt
							# cat $INDEL_dir/join5.txt | tr " " "\t" > $INDEL_dir/join6.txt
							
							# touch $INDEL_dir/$sample.INDEL.summary
							# echo -e "CODING\tFRAMESHIFT\tSPLICE-3\tSPLICE-5\tUTR-3\tUTR-5" >> $INDEL_dir/$sample.INDEL.summary
							# cat $INDEL_dir/join6.txt >> $INDEL_dir/$sample.INDEL.summary
							# Rscript $script_path/sum.cols.r $INDEL_dir/$sample.INDEL.summary $INDEL_dir/$sample.INDEL.sum
							
							# rm $INDEL_dir/*.tmp $INDEL_dir/join*.txt $INDEL_dir/$sample.coding.txt $INDEL_dir/$sample.frameshift.txt $INDEL_dir/$sample.splice3.txt $INDEL_dir/$sample.splice5.txt $INDEL_dir/$sample.utr3.txt $INDEL_dir/$sample.utr5.txt		
				###########################################################################################			### generating gene summary file
							# cat $master_gene_file | awk '{print $4"\t"$1"\t"$2"\t"$3"\t"$5}' > $report_dir/GeneList.forsummary.txt
							# cat $master_entrez_file | awk '{print $2}' > $report_dir/EntrezID.txt
							# touch $report_dir/$sample.Gene.Summary.txt
							# echo -e "\t\t\t\t\t\t\t\t\t\t$sample" >> $report_dir/$sample.Gene.Summary.txt
							# echo -e "\t\t\t\t\t\t\t\t\t\tSNV_breakdown\t\t\t\t\t\t\t\tINDEL_breakdown\t\t\t\t\t\t" >> $report_dir/$sample.Gene.Summary.txt
							# echo -e "Gene\tChromosome\tStart\tStop\tStrand\tEntrez_Gene_ID\tTotal_SNVs\tTotal_INDELs\tTotal_CNVs\tTotal_SVs\tNONSENSE\tMISSENSE\tCODING-SYNONYMOUS\tCODING-NOTMOD3\tSPLICE-3\tSPLICE-5\tUTR-3\tUTR-5\tCODING\tFRAMESHIFT\tSPLICE-3\tSPLICE-5\tUTR-3\tUTR-5" >> $report_dir/$sample.Gene.Summary.txt
							# sed -i '1d' $SNV_dir/$sample.SNV.summary
							# sed -i '1d' $INDEL_dir/$sample.INDEL.summary
							# cat $SNV_dir/$sample.SNV.summary | cut -f2,3,4,5,6,7,8,9 > $SNV_dir/$sample.SNV.tmp
							# cat $INDEL_dir/$sample.INDEL.summary | cut -f2,3,4,5,6,7,8,9 > $INDEL_dir/$sample.INDEL.tmp
							...
							# rm $SNV_dir/$sample.SNV.tmp $INDEL_dir/$sample.INDEL.tmp $report_dir/gene.temp $report_dir/GeneList.forsummary.txt $INDEL_dir/$sample.INDEL.summary $INDEL_dir/$sample.INDEL.sum $SNV_dir/$sample.SNV.summary $SNV_dir/$sample.SNV.sum $report_dir/EntrezID.txt
							
				# else
					# echo "Whole Genome Analysis"
							# for group in `echo $groups | tr ":" " "`
				# do
					# sampleNames=$( cat $sample_info | grep -w "^$group" | cut -d '=' -f2)
				...
				##########################################################################################	
							## summarizing CNV files
							# cat $CNV_dir/$sample.CNV.annotated.txt | tr "_" "\t" | cut -f4,10 | grep DUP > $CNV_dir/$sample.DUP.tmp
							# cat $CNV_dir/$sample.CNV.annotated.txt | tr "_" "\t" | cut -f4,10 | grep DEL > $CNV_dir/$sample.DEL.tmp
							
							# Rscript $script_path/summary.CNV.r $report_dir/gene.temp $CNV_dir/$sample.DEL.tmp $CNV_dir/$sample.DUP.tmp $CNV_dir/$sample.DEL.txt $CNV_dir/$sample.DUP.txt
							
							# join $CNV_dir/$sample.DEL.txt $CNV_dir/$sample.DUP.txt > $CNV_dir/join.txt
							# cat $CNV_dir/join.txt | tr " " "\t" > $CNV_dir/join1.txt
							
							# touch $CNV_dir/$sample.CNV.summary
							# echo -e "DEL\tDUP" >> $CNV_dir/$sample.CNV.summary
							# cat $CNV_dir/join1.txt >> $CNV_dir/$sample.CNV.summary
							# Rscript $script_path/sum.cols.r $CNV_dir/$sample.CNV.summary $CNV_dir/$sample.CNV.sum
							
							# rm $CNV_dir/*.tmp $CNV_dir/join*.txt $CNV_dir/$sample.DEL.txt $CNV_dir/$sample.DUP.txt
				###########################################################################################			### summarizing SV files
							# cat $SV_dir/$sample.SV.annotated.txt | sed -e '/NA_/s//NOGENE_/g' -e '/_NA/s//_NOGENE/g' | grep INV | tr "_" "\t" | cut -f9,10,11 | tr " " "\t" > $SV_dir/$sample.INV.tmp
							# cat $SV_dir/$sample.SV.annotated.txt | sed -e '/NA_/s//NOGENE_/g' -e '/_NA/s//_NOGENE/g' | grep INS | tr "_" "\t" | cut -f9,10,11 | tr " " "\t" > $SV_dir/$sample.INS.tmp
							# cat $SV_dir/$sample.SV.annotated.txt | sed -e '/NA_/s//NOGENE_/g' -e '/_NA/s//_NOGENE/g' | grep DEL | tr "_" "\t" | cut -f9,10,11 | tr " " "\t" > $SV_dir/$sample.DEL.tmp
							# cat $SV_dir/$sample.SV.annotated.txt | sed -e '/NA_/s//NOGENE_/g' -e '/_NA/s//_NOGENE/g' | grep ITX | tr "_" "\t" | cut -f9,10,11 | tr " " "\t" > $SV_dir/$sample.ITX.tmp
							# cat $SV_dir/$sample.SV.annotated.txt | sed -e '/NA_/s//NOGENE_/g' -e '/_NA/s//_NOGENE/g' | grep CTX | tr "_" "\t" | cut -f9,10,11 | tr " " "\t" > $SV_dir/$sample.CTX.tmp
							
							# Rscript $script_path/summary.SV.r $report_dir/gene.temp $SV_dir/$sample.ITX.tmp $SV_dir/$sample.INV.tmp $SV_dir/$sample.DEL.tmp $SV_dir/$sample.INS.tmp $SV_dir/$sample.CTX.tmp $SV_dir/$sample.ITX.txt $SV_dir/$sample.INV.txt $SV_dir/$sample.DEL.txt $SV_dir/$sample.INS.txt $SV_dir/$sample.CTX.txt
							
							# join $SV_dir/$sample.ITX.txt $SV_dir/$sample.INV.txt > $SV_dir/join1.txt
							# join $SV_dir/join1.txt $SV_dir/$sample.DEL.txt > $SV_dir/join2.txt
							# join $SV_dir/join2.txt $SV_dir/$sample.INS.txt > $SV_dir/join3.txt
							# join $SV_dir/join3.txt $SV_dir/$sample.CTX.txt > $SV_dir/join4.txt
							# cat $SV_dir/join4.txt | tr " " "\t" > $SV_dir/join5.txt
							
							# touch $SV_dir/$sample.SV.summary
							# echo -e "ITX\tINV\tDEL\tINS\tCTX" >> $SV_dir/$sample.SV.summary
							# cat $SV_dir/join5.txt >> $SV_dir/$sample.SV.summary
							# Rscript $script_path/sum.cols.r $SV_dir/$sample.SV.summary $SV_dir/$sample.SV.sum
							
							# rm $SV_dir/*.tmp $SV_dir/join*.txt $SV_dir/$sample.ITX.txt $SV_dir/$sample.INV.txt $SV_dir/$sample.DEL.txt $SV_dir/$sample.INS.txt $SV_dir/$sample.CTX.txt 
				###########################################################################################			### generating gene summary file
							# cat $master_gene_file | awk '{print $4"\t"$1"\t"$2"\t"$3"\t"$5}' > $report_dir/GeneList.forsummary.txt
							# cat $master_entrez_file | awk '{print $2}' > $report_dir/EntrezID.txt
							# touch $report_dir/$sample.Gene.Summary.txt
							...
							# paste $report_dir/GeneList.forsummary.txt $report_dir/EntrezID.txt $SNV_dir/$sample.SNV.sum $INDEL_dir/$sample.INDEL.sum $CNV_dir/$sample.CNV.sum $SV_dir/$sample.SV.sum $SNV_dir/$sample.SNV.tmp $INDEL_dir/$sample.INDEL.tmp $CNV_dir/$sample.CNV.tmp $SV_dir/$sample.SV.tmp >> $report_dir/$sample.Gene.Summary.txt
							
							# rm $SNV_dir/$sample.SNV.tmp $INDEL_dir/$sample.INDEL.tmp $CNV_dir/$sample.CNV.tmp $SV_dir/$sample.SV.tmp $report_dir/gene.temp $report_dir/GeneList.forsummary.txt $SV_dir/$sample.SV.summary $SV_dir/$sample.SV.sum $CNV_dir/$sample.CNV.summary $CNV_dir/$sample.CNV.sum $INDEL_dir/$sample.INDEL.summary $INDEL_dir/$sample.INDEL.sum $SNV_dir/$sample.SNV.summary $SNV_dir/$sample.SNV.sum $report_dir/EntrezID.txt
						# fi
					# done
				# done
			# fi
