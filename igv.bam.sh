...
if [ $# != 4 ];
then
    echo -e "Usage: SCRIPT to create IGV BAM \n</path/to/realign dir> </path/to/output folder> <sample> <run ifno>";
else	
    set -x
    echo `date`
    input=$1
    ...
    then
        ...
        do
            ...
            then
                ...
                do
                    chr=${chrArray[$i]}
                    $samtools/samtools view -b -r $i $input/$sample//chr${chrArray[$i]}.cleaned.bam > $output/$sample.$i.chr$chr.bam
                ...
                $samtools/samtools view -b -r $i $input/$sample//chr${chrArray[$i]}.cleaned.bam > $output/$i.igv-sorted.bam
                $samtools/samtools reheader $output/$sample.$i.header.sam $output/$i.igv-sorted.bam > $output/$i.igv-sorted.re.bam
