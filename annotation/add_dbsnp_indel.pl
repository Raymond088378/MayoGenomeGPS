...
our ($opt_i, $opt_s, $opt_c, $opt_p, $opt_x, $opt_b, $opt_o);
...
unless ( (defined $opt_i) && (defined $opt_s) && (defined $opt_c) && (defined $opt_p) && (defined $opt_x) && (defined $opt_b) && (defined $opt_o) ){
	die ("Usage: $0 [-i input file containing chr and pos in tab delimited format -full path]  [-b pos base in input file, 1 for 1-based, 0 for 0-based] [-s dbSNP UCSChg##dbSNP### file use chromStart 0-based, current rsids only] [-c column# of chr in the input file] [-p column# of start ] [-x column# of stop] [-o output file path]\n");
}
