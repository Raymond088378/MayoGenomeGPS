...
our ($opt_i, $opt_c, $opt_p, $opt_b, $opt_e, $opt_g, $opt_s, $opt_o);
#print "Raw parameters: @ARGV\n";
getopt('igscpbeo');

unless ( (defined $opt_i) && (defined $opt_s) && (defined $opt_c) && (defined $opt_p) && (defined $opt_b) && (defined $opt_o) ){
	die ("Usage: $0 [-i input file containing chr and pos in tab delimited format -full path][-c column# of chr in the input file] [-p column# of pos ] [-b pos base, 1 for 1-based, 0 for 0-based] [-e population, CEU, YRI or JPT+CHB] [-s hapmap file (0-based) including all chr] [-g 1000genome file (0-based) -full path] [-o output file path]\n");
}
