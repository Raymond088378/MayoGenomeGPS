...
while(my $l = <FH>){
	...
	while(my $k = <FILE>){
		...
		if ( $. == 1){
			...
			if ($id ne $prev)	{
				my $num_iterations=$i/$num_samples;
				...
				if ($value == $num_iterations){
					...
					for(my $ll=$value; $ll <$num_iterations;$ll++){
						$sam="n/a\tn/a\tn/a\tn/a\tn/a\tn/a\t" x $num_samples; 
		 				$sam =~ s/\s+$//;
	...
	foreach my $p (sort {$a<=> $b} keys %{$ref{$c}})	{
		foreach my $a (sort keys %{$ref{$c}{$p}})	{	
			...
			for(my $rows=0; $rows < $num_snpeff; $rows++)	{
				...
				my $values=@val1;
				$values=$values/6;
				##print "@{$sample_info{$c}{$p}{$a}}\n";
				#print "$values\t$num_samples\t$count\n";
				#<STDIN>;
