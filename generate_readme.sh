...
then
	...
	then
		...
		do
			...
			echo -e "unfiltered multi allelic somatic variant file for $pair : $pair.somatic.variants.raw.multi.vcf" >> $file
			echo -e "filtered multi sample variant file for $pair : $pair.variants.filter.vcf" >> $file
			echo -e "unfiltered multi allelic multi sample variant file for $pair : $pair.variants.raw.multi.vcf" >> $file
		done
	else
		for sam in $samples
		do			
			echo -e "\nsample : $sam" >> $file
			echo -e "filtered variant file for $sam : $sam.variants.raw.multi.vcf" >> $file
			echo -e "unfiltered multi allelic variant file for $sam :$sam.variants.filter.vcf" >> $file
		done
	fi
	if [ $multi == "NO" ]
	then
