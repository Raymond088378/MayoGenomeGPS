...
if ( (!defined $opt_i) && (!defined $opt_c) && (!defined $opt_o) ) {
	...
	close LIST;
	print OUT "\n";
	print OUT "Chr\tPosition\tInCaptureKit\tRef\tAlt\t"; ## Output format
	my %snv;
	my $i = 0; ## Track of which sample the call comes from; to make sure newer indel calls get adequate tabs
	open LIST, "<$source" or die "can not open $source : $! \n"; ## List of file-names to merge...
	foreach my $f (<LIST>)	{
		chomp $f;
	    print OUT "GenotypeClass\tAlt-SupportedReads\tRef-SupportedReads\tReadDepth\t${last_col}\tCloseToIndel\t"; ## Output format
		my $list = '';
