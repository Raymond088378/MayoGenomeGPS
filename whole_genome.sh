...
then	
	...
	then
		...
		do            
			...
			then
				...
				then
					...
					qsub $args -N $type.$version.align_novo.$sample.$run_num -l h_vmem=$mem -pe threaded $threads -t 1-$numfiles:1 $script_path/align_novo.sh $sample $output_dir $run_info
				elif [ $aligner == "bwa" ]
				then
					echo "bwa is used as aligner"
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^align_read_bwa' | cut -d '=' -f2)
					qsub $args -N $type.$version.align_read_bwa.R1.$sample.$run_num -l h_vmem=$mem -pe threaded $threads -t 1-$numfiles:1 $script_path/align_read_bwa.sh $sample $output_dir 1 $run_info
					if [ $paired == 1 ]
					then
						$script_path/check_qstat.sh $limit
						mem=$( cat $memory_info | grep -w '^align_read_bwa' | cut -d '=' -f2)
						qsub $args -N $type.$version.align_read_bwa.R2.$sample.$run_num -l h_vmem=$mem -pe threaded $threads -t 1-$numfiles:1 $script_path/align_read_bwa.sh $sample $output_dir 2 $run_info
					...
					qsub $args -N $type.$version.align_bwa.$sample.$run_num -l h_vmem=$mem -t 1-$numfiles:1 $script_path/align_bwa.sh $sample $output_dir $run_info
				else
				...
				qsub $args -N $type.$version.processBAM.$sample.$run_num -l h_vmem=$mem $hold $script_path/processBAM.sh $align_dir $sample $run_info 	
				if [ $analysis != "alignment" ]
				then
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^extract_reads_bam' | cut -d '=' -f2)
					qsub $args -N $type.$version.extract_reads_bam.$sample.$run_num -l h_vmem=$mem -hold_jid $type.$version.processBAM.$sample.$run_num $script_path/extract_reads_bam.sh $align_dir $bamfile $run_info $igv
				fi
			...
			then
				...
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^processBAM' | cut -d '=' -f2)
				qsub $args -N $type.$version.processBAM.$sample.$run_num -l h_vmem=$mem $script_path/processBAM.sh $align_dir $sample $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^extract_reads_bam' | cut -d '=' -f2)
				qsub $args -N $type.$version.extract_reads_bam.$sample.$run_num -l h_vmem=$mem -hold_jid $type.$version.processBAM.$sample.$run_num $script_path/extract_reads_bam.sh $align_dir $bamfile $run_info $igv
			fi    
			...
			then
				...
				then
					...
					qsub $args -N $type.$version.reformat_BAM.$sample.$run_num -l h_vmem=$mem $script_path/reformat_BAM.sh $realign_dir $sample $run_info	
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^extract_reads_bam' | cut -d '=' -f2)
					qsub $args -N $type.$version.extract_reads_bam.$sample.$run_num -l h_vmem=$mem -hold_jid $type.$version.reformat_BAM.$sample.$run_num $script_path/extract_reads_bam.sh $realign_dir $bamfile $run_info $igv
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^split_bam_chr' | cut -d '=' -f2)
					qsub $args -N $type.$version.split_bam_chr.$sample.$run_num -hold_jid $type.$version.reformat_BAM.$sample.$run_num -l h_vmem=$mem -t 1-$numchrs:1 $script_path/split_bam_chr.sh $realign_dir $sample $run_info
					variant_id="$type.$version.split_bam_chr.$sample.$run_num"
				else
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^realign_recal' | cut -d '=' -f2)
					qsub $args -N $type.$version.realign_recal.$sample.$run_num -hold_jid $type.$version.processBAM.$sample.$run_num -l h_vmem=$mem -t 1-$numchrs:1 $script_path/realign_recal.sh $align_dir $bamfile $sample $realign_dir $run_info 1	
					variant_id="$type.$version.realign_recal.$sample.$run_num"
				fi
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^igv_bam' | cut -d '=' -f2)
				qsub $args -N $type.$version.igv_bam.$sample.$run_num -l h_vmem=$mem -hold_jid $variant_id,$type.$version.extract_reads_bam.$sample.$run_num $script_path/igv_bam.sh $output_realign $igv $sample $output_align $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^variants' | cut -d '=' -f2)
				qsub $args -N $type.$version.variants.$sample.$run_num -hold_jid $variant_id -pe threaded $threads -l h_vmem=$mem -t 1-$numchrs:1 $script_path/variants.sh $realign_dir $sample $variant_dir 1 $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^merge_variant_single' | cut -d '=' -f2)
				qsub $args -N $type.$version.merge_variant_single.$sample.$run_num -l h_vmem=$mem -pe threaded $threads -hold_jid $type.$version.variants.$sample.$run_num $script_path/merge_variant_single.sh $output_variant $sample $RSample $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^OnTarget_BAM' | cut -d '=' -f2)
				qsub $args -N $type.$version.OnTarget_BAM.$sample.$run_num -hold_jid $variant_id -l h_vmem=$mem -t 1-$numchrs:1 $script_path/OnTarget_BAM.sh $realign_dir $output_OnTarget $sample $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^OnTarget_PILEUP' | cut -d '=' -f2)
				qsub $args -N $type.$version.OnTarget_PILEUP.$sample.$run_num -hold_jid $variant_id -l h_vmem=$mem -t 1-$numchrs:1 $script_path/OnTarget_PILEUP.sh $realign_dir $output_OnTarget $sample $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^getCoverage' | cut -d '=' -f2)
				qsub $args -N $type.$version.getCoverage.$sample.$run_num -hold_jid $type.$version.OnTarget_PILEUP.$sample.$run_num -l h_vmem=$mem $script_path/getCoverage.sh $output_OnTarget $numbers $sample $run_info    
			fi
			...
			then
				...
				then
					...
					qsub $args -N $type.$version.reformat_VARIANTs_OnTarget.$sample.$run_num -l h_vmem=$mem $script_path/reformat_VARIANTs_OnTarget.sh $output_variant $RSample $sample $run_info 2
				elif [ $variant_type == "SNV" -o $variant_type == "INDEL" ]
				then
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^reformat_VARIANTs_OnTarget' | cut -d '=' -f2)
					qsub $args -N $type.$version.reformat_VARIANTs_OnTarget.$sample.$run_num -l h_vmem=$mem $script_path/reformat_VARIANTs_OnTarget.sh $output_variant $RSample $sample $run_info 1
				fi
			...
			then
				...
				qsub $args -N $type.$version.OnTarget_variant.$sample.$run_num -t 1-$numchrs:1 $hold_args -l h_vmem=$mem $script_path/OnTarget_variant.sh $output_variant $output_OnTarget $sample $run_info
			fi
			if [ $analysis == "annotation" ]
			then
				if [ $variant_type == "BOTH" ]
				then
					...
					qsub $args -N $type.$version.reformat_VARIANTs.$sample.$run_num -l h_vmem=$mem $script_path/reformat_VARIANTs.sh $output_OnTarget $sample $run_info 2
				elif [ $variant_type == "SNV" -o $variant_type == "INDEL" ]
				then
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^reformat_VARIANTs' | cut -d '=' -f2)
					qsub $args -N $type.$version.reformat_VARIANTs.$sample.$run_num -l h_vmem=$mem $script_path/reformat_VARIANTs.sh $output_OnTarget $sample $run_info 1
				fi
			...
			then
				...
				then
					...
					qsub $args -N $type.$version.sift.$sample.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/sift.sh $sift $output_OnTarget $sample $run_info
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^polyphen' | cut -d '=' -f2)
					qsub $args -N $type.$version.polyphen.$sample.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/polyphen.sh $polyphen $output_OnTarget $sample $run_info	    	
				fi
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^snpeff' | cut -d '=' -f2)
				qsub $args -N $type.$version.snpeff.$sample.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/snpeff.sh $snpeff $output_OnTarget $sample $run_info		
				if [ $variant_type == "SNV" -o $variant_type == "BOTH" ]
				then
				...
				qsub $args -N $type.$version.sample_reports.$sample.$run_num $hold -t 1-$numchrs:1 -l h_vmem=$mem $script_path/reports.sh $run_info $sample $TempReports $output_OnTarget $sift $snpeff $polyphen $output_dir
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^sample_reports' | cut -d '=' -f2)
				qsub $args -N $type.$version.sample_report.$sample.$run_num -hold_jid $type.$version.sample_reports.$sample.$run_num -l h_vmem=$mem $script_path/sample_reports.sh $output_dir $TempReports $sample $run_info
				...
				then
					...
					qsub $args -N $type.$version.run_single_crest.sh.$sample.$run_num -hold_jid $type.$version.processBAM.$sample.$run_num -t 1-$numchrs:1 -l h_vmem=$mem $script_path/run_single_crest.sh $sample $align_dir $bamfile $crest $run_info
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^run_cnvnator' | cut -d '=' -f2)
					qsub $args -N $type.$version.run_cnvnator.$sample.$run_num -hold_jid $variant_id -l h_vmem=$mem -t 1-$numchrs:1 $script_path/run_cnvnator.sh $sample $realign_dir $cnv $run_info
					let nump=$numchrs+1;
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^run_breakdancer' | cut -d '=' -f2)
					qsub $args -N $type.$version.run_breakdancer.$sample.$run_num -hold_jid $variant_id -l h_vmem=$mem -t 1-$numchrs:1 $script_path/run_breakdancer.sh $sample $output_realign $break $run_info
					$script_path/check_qstat.sh $limit
					qsub $args -N $type.$version.run_breakdancer_in.$sample.$run_num -hold_jid $type.$version.igv_bam.$sample.$run_num -l h_vmem=$mem -t $nump-$nump:$nump $script_path/run_breakdancer.sh $sample $igv $break $run_info
					...
					qsub $args -N $type.$version.summaryze_struct_single.$sample.$run_num -l h_vmem=$mem $hold $script_path/summaryze_struct_single.sh $sample $output_dir $run_info
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^plot_circos_cnv_sv' | cut -d '=' -f2)
					qsub $args -N $type.$version.plot_circos_cnv_sv.$sample.$run_num -hold_jid $type.$version.summaryze_struct_single.$sample.$run_num -l h_vmem=$mem $script_path/plot_circos_cnv_sv.sh $break/$sample/$sample.break $crest/$sample/$sample.filter.crest $cnv/$sample.cnv.filter.bed $sample $output_dir/circos $run_info	
				fi
				...
				then
					...
					qsub $args -N $type.$version.annotation_CNV.$sample.$run_num -l h_vmem=$mem -hold_jid $type.$version.plot_circos_cnv_sv.$sample.$run_num $script_path/annotation_CNV.sh $sv $run_info $annot $sample
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^annotation_SV' | cut -d '=' -f2)
					qsub $args -N $type.$version.annotation_SV.sh.$sample.$run_num -l h_vmem=$mem -hold_jid $type.$version.plot_circos_cnv_sv.$sample.$run_num $script_path/annotation_SV.sh $output_dir $run_info $annot $sample
				fi	
			fi
			...
			qsub $args -N $type.$version.sample_numbers.$sample.$run_num $hold_args -l h_vmem=$mem $script_path/sample_numbers.sh $output_dir $sample $run_info $numbers
			if [ $analysis != "alignment" ]
			then
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^gene_summary' | cut -d '=' -f2)
				qsub $args -N $type.$version.gene_summary.$sample.$run_num $hold_args -l h_vmem=$mem $script_path/gene_summary.sh $output_dir $sample $run_info $RSample		
			fi
		done
		### concat raw varaints
		...
		then
			...
			qsub $args -N $type.$version.merge_raw_variants.$run_num -t 1-$numchrs:1 -hold_jid $id -l h_vmem=$mem $script_path/merge_raw_variants.sh $output_dir $run_info
			$script_path/check_qstat.sh $limit
			mem=$( cat $memory_info | grep -w '^concat_raw_variants' | cut -d '=' -f2)
			qsub $args -N $type.$version.concat_raw_variants.$run_num -hold_jid $type.$version.merge_raw_variants.$run_num -l h_vmem=$mem $script_path/concat_raw_variants.sh $output_dir $run_info	    
		fi	
		if [ $analysis != "alignment" ]
		then
			id=""
			...
			qsub $args -N $type.$version.merge_sample.$run_num -hold_jid $id -l h_vmem=$mem $script_path/merge_sample.sh $output_dir $run_info    
		fi
		id_igv=""
		id_numbers=""
		id_gene_summary=""
		...
		do
			...
			then
				...
				then
					...
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^align_bwa' | cut -d '=' -f2)
					qsub $args -N $type.$version.align_bwa.$sample.$run_num -l h_vmem=$mem $hold -t 1-$numfiles:1 $script_path/align_bwa.sh $sample $output_dir $run_info
					hold="$type.$version.align_bwa.$sample.$run_num"
				fi	    
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^processBAM' | cut -d '=' -f2)
				qsub $args -N $type.$version.processBAM.$sample.$run_num -l h_vmem=$mem -hold_jid $hold $script_path/processBAM.sh $align_dir $sample $run_info $script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^extract_reads_bam' | cut -d '=' -f2)
				qsub $args -N $type.$version.extract_reads_bam.$sample.$run_num -l h_vmem=$mem -hold_jid $type.$version.processBAM.$sample.$run_num $script_path/extract_reads_bam.sh $align_dir $bamfile $run_info $igv		
			elif [[ $analysis == "realignment" || $analysis == "realign-mayo" ]]
			then
		...
		then
			...
			do
				...
				then
					...
					qsub $args -N $type.$version.reformat_pairBAM.$group.$run_num -l h_vmem=$mem $script_path/reformat_pairBAM.sh $realign_dir $group $run_info
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^extract_reads_bam' | cut -d '=' -f2)
					qsub $args -N $type.$version.extract_reads_bam.$group.$run_num -l h_vmem=$mem -hold_jid $type.$version.reformat_pairBAM.$group.$run_num $script_path/extract_reads_bam.sh $realign_dir $group.sorted.bam $run_info $igv $group
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^split_bam_chr' | cut -d '=' -f2)
					qsub $args -N $type.$version.split_bam_chr.$group.$run_num -hold_jid $type.$version.reformat_pairBAM.$group.$run_num -l h_vmem=$mem -t 1-$numchrs:1 $script_path/split_bam_chr.sh $realign_dir $group $run_info
				...
				qsub $args -N $type.$version.split_sample_pair.$group.$run_num -hold_jid $variant_id -l h_vmem=$mem -t 1-$numchrs:1 $script_path/split_sample_pair.sh $output_realign $igv $group $output_align $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^igv_bam' | cut -d '=' -f2)
				qsub $args -N $type.$version.igv_bam.$group.$run_num -hold_jid $type.$version.split_sample_pair.$group.$run_num -l h_vmem=$mem $script_path/igv_bam.sh $output_realign $igv $group $output_align $run_info 
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^variants' | cut -d '=' -f2)
				qsub $args -N $type.$version.variants.$group.$run_num -hold_jid $variant_id -pe threaded $threads -l h_vmem=$mem -t 1-$numchrs:1 $script_path/variants.sh $realign_dir $names_samples $variant_dir 1 $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^merge_variant_group' | cut -d '=' -f2)
				qsub $args -N $type.$version.merge_variant_group.$group.$run_num -l h_vmem=$mem -pe threaded $threads -hold_jid $type.$version.variants.$group.$run_num $script_path/merge_variant_group.sh $output_variant $group $RSample $run_info 
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^OnTarget_BAM' | cut -d '=' -f2)
				qsub $args -N $type.$version.OnTarget_BAM.$group.$run_num -hold_jid $type.$version.split_sample_pair.$group.$run_num -l h_vmem=$mem -t 1-$numchrs:1 $script_path/OnTarget_BAM.sh $igv $output_OnTarget $group $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^OnTarget_PILEUP' | cut -d '=' -f2)
				qsub $args -N $type.$version.OnTarget_PILEUP.$group.$run_num -hold_jid $type.$version.split_sample_pair.$group.$run_num -l h_vmem=$mem -t 1-$numchrs:1 $script_path/OnTarget_PILEUP.sh $realign_dir $output_OnTarget $group $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^getCoverage' | cut -d '=' -f2)
				qsub $args -N $type.$version.getCoverage.$group.$run_num -hold_jid $type.$version.OnTarget_PILEUP.$group.$run_num -l h_vmem=$mem $script_path/getCoverage.sh $output_OnTarget $numbers $group $run_info
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^OnTarget_variant' | cut -d '=' -f2)
				qsub $args -N $type.$version.OnTarget_variant.$group.$run_num -l h_vmem=$mem -hold_jid $type.$version.merge_variant_group.$group.$run_num -t 1-$numchrs:1 $script_path/OnTarget_variant.sh $output_variant $output_OnTarget $group $run_info
				hold_args="-hold_jid $type.$version.OnTarget_variant.$group.$run_num"
				## SIFT
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^sift' | cut -d '=' -f2)
				qsub $args -N $type.$version.sift.$group.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/sift.sh $sift $output_OnTarget $group $run_info 
				$script_path/check_qstat.sh $limit
				qsub $args -N $type.$version.sift.$group.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/sift.sh $sift $output_OnTarget $group $run_info TUMOR
				## SNPEFF
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^snpeff' | cut -d '=' -f2)
				qsub $args -N $type.$version.snpeff.$group.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/snpeff.sh $snpeff $output_OnTarget $group $run_info
				$script_path/check_qstat.sh $limit
				qsub $args -N $type.$version.snpeff.$group.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/snpeff.sh $snpeff $output_OnTarget $group $run_info TUMOR
				##POLYPHEN
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^polyphen' | cut -d '=' -f2)
				qsub $args -N $type.$version.polyphen.$group.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/polyphen.sh $polyphen $output_OnTarget $group $run_info  
				$script_path/check_qstat.sh $limit
				qsub $args -N $type.$version.polyphen.$group.$run_num $hold_args -t 1-$numchrs:1 -l h_vmem=$mem $script_path/polyphen.sh $polyphen $output_OnTarget $group $run_info TUMOR
				hold="$type.$version.sift.$group.$run_num,$type.$version.snpeff.$group.$run_num,$type.$version.polyphen.$group.$run_num"
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^reports' | cut -d '=' -f2)
				qsub $args -N $type.$version.sample_reports.$group.$run_num -hold_jid $hold -t 1-$numchrs:1 -l h_vmem=$mem $script_path/reports.sh $run_info $group $TempReports $output_OnTarget $sift $snpeff $polyphen $output_dir
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^sample_report' | cut -d '=' -f2)
				qsub $args -N $type.$version.sample_report.$sample.$run_num -l h_vmem=$mem -hold_jid $type.$version.sample_reports.$group.$run_num $script_path/sample_report.sh $output_dir $TempReports $group $run_info
				hold="$type.$version.sift.${group}.$run_num,$type.$version.snpeff.$group.$run_num,$type.$version.polyphen.$group.$run_num"
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^reports' | cut -d '=' -f2)
				qsub $args -N $type.$version.sample_reports.$group.$run_num -hold_jid $hold -t 1-$numchrs:1 -l h_vmem=$mem $script_path/reports.sh $run_info $group $TempReports $output_OnTarget $sift $snpeff $polyphen $output_dir TUMOR
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^sample_report' | cut -d '=' -f2)
				qsub $args -N $type.$version.sample_report.$group.$run_num -l h_vmem=$mem -hold_jid $type.$version.sample_reports.$group.$run_num $script_path/sample_report.sh $output_dir $TempReports $group $run_info TUMOR
				if [ $tool == "whole_genome" ]
				then
					crest=$output_dir/struct/crest
					break=$output_dir/struct/break
					...
					qsub $args -N $type.$version.run_crest_multi.$group.$run_num -hold_jid $id -l h_vmem=$mem -t 1-$numchrs:1 $script_path/run_crest_multi.sh $group $igv $crest $run_info
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^run_segseq' | cut -d '=' -f2)
					qsub $args -N $type.$version.run_segseq.$group.$run_num -hold_jid $type.$version.split_sample_pair.$group.$run_num -l h_vmem=$mem -t 1-$numchrs:1 -l matlab_lic=1 $script_path/run_segseq.sh $group $igv $output_dir/cnv $run_info    
					let nump=$numchrs+1;    
					mkdir -p $break/$group
					id=""
					for sam in `cat $sample_info| grep -w "^$group" | cut -d '=' -f2`
					do
						$script_path/check_qstat.sh $limit
						mem=$( cat $memory_info | grep -w '^run_breakdancer' | cut -d '=' -f2)
						qsub $args -N $type.$version.run_breakdancer.$group.$sam.$run_num -hold_jid $type.$version.split_sample_pair.$group.$run_num -l h_vmem=$mem -t 1-$numchrs:1 $script_path/run_breakdancer.sh $sam $igv $break/$group $run_info $group
						$script_path/check_qstat.sh $limit
						qsub $args -N $type.$version.run_breakdancer_in.$group.$sam.$run_num -hold_jid $type.$version.igv_bam.$group.$run_num -l h_vmem=$mem -t $nump-$nump:$nump $script_path/run_breakdancer.sh $sam $igv $break/$group $run_info $group
					...
					qsub $args -N $type.$version.summaryze_struct_group.$group.$run_num -l h_vmem=$mem -hold_jid $hhold $script_path/summaryze_struct_group.sh $group $output_dir $run_info
					...
					do  
						...
						$script_path/check_qstat.sh $limit
						mem=$( cat $memory_info | grep -w '^plot_circos_cnv_sv' | cut -d '=' -f2)
						qsub $args -N $type.$version.plot_circos_cnv_sv.$group.$tumor.$i.$run_num -hold_jid $type.$version.summaryze_struct_group.$group.$run_num -l h_vmem=$mem $script_path/plot_circos_cnv_sv.sh $output_dir/struct/$group.$tumor.somatic.break $output_dir/struct/$group.$tumor.somatic.filter.crest $output_dir/cnv/$group/$tumor.cnv.filter.bed $group.$tumor $output_dir/circos $run_info
					done
				fi
			done
			mkdir -p $annot
			...
			then
				...
				do
					...
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^annotation_CNV' | cut -d '=' -f2)
					qsub $args -N $type.$version.annotation_CNV.$group.$run_num -l h_vmem=$mem -hold_jid $id $script_path/annotation_CNV.sh $sv $run_info $annot $group
					$script_path/check_qstat.sh $limit
					mem=$( cat $memory_info | grep -w '^annotation_SV' | cut -d '=' -f2)
					qsub $args -N $type.$version.annotation_SV.$group.$run_num -l h_vmem=$mem -hold_jid $id $script_path/annotation_SV.sh $output_dir $run_info $annot $group
				done
			fi
			...
			do
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^sample_numbers' | cut -d '=' -f2)
				qsub $args -N $type.$version.sample_numbers.$group.$run_num -l h_vmem=$mem -hold_jid $id $script_path/sample_numbers.sh $output_dir $group $run_info $numbers
				$script_path/check_qstat.sh $limit
				mem=$( cat $memory_info | grep -w '^gene_summary' | cut -d '=' -f2)
				qsub $args -N $type.$version.gene_summary.$group.$run_num -l h_vmem=$mem -hold_jid $id $script_path/gene_summary.sh $output_dir $group $run_info $RSample
			done
