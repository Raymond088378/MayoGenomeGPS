...
then	
	...
	##
	SPLITTING=$(perl $DIR/Scripts/splitTREAT.pl $INPUT $FEATURE $TMPDIR $SPLIT  2>&1)
	if [  "$SPLITTING" != "" ]
	then
		echo -e "ERROR:\n$SPLITTING"
		rm -R $TMPDIR
	...
	sendtoSGE(){
		...
		OPTS=`echo $3 | tr "+" " "`
		echo "$EXE $DIR/Scripts/$2 $OPTS" >> $TMPNAME
...
exit 0;
	...
	GENEHOLD=$(sendtoSGE perl myGeneFinder.pl $DIR/db/ucsc_refflat_hg19_2011-01-24.bed+$TMPDIR/VARFILE_\$SGE_TASK_ID 2G 1);
		
	###
	# Classify if they are known pathogenic variants (Boolean)
	P1_HOLD=$(sendtoSGE perl pharmicoBool.pl $TMPDIR/VARFILE_\$SGE_TASK_ID+$DIR/db/rsidSlim.tsv 2.5G 1);
	
	###
	# Query for HGMD Variant Information
	H1_HOLD=$(sendtoSGE perl hgmdQuery_var.pl $TMPDIR/VARFILE_\$SGE_TASK_ID 400M 1);
	...
	B2_HOLD=$(sendtoSGE java DrugTargetSingle_fat.jar -i+$TMPDIR/VARFILE_\${SGE_TASK_ID}_GENE+-o+$TMPDIR/VARFILE_\${SGE_TASK_ID}_BEAUTY+-db+$DIR/db/druggable.sqlite 3G 1 $GENEHOLD);
	
	###
	# Query for Drugs, within a pathway of desired Gene
	DP2_HOLD=$(sendtoSGE java DrugTargetPathway.jar -i+$TMPDIR/VARFILE_\${SGE_TASK_ID}_GENE+-o+$TMPDIR/VARFILE_\${SGE_TASK_ID}_PATHWAY+-db+$DIR/db/druggable2.sqlite 3G 1 $GENEHOLD);
	...
	DG2_HOLD=$(sendtoSGE perl druggable_flag.pl $TMPDIR/VARFILE_\${SGE_TASK_ID}_GENE+$DIR/db/DruggableGenome.txt 1G 1 $GENEHOLD);
	...
	## need to put into SGE  rm -R $TMPDIR
	
	
	### Convert unix tabbed file into excel + format requested....flexible
fi
