...
if ( (!defined $opt_i) && (!defined $opt_o) ) {
	...
	foreach my $f (<LIST>)	{
    	...
    	open (F, $f);
    	while (my $l = <F>) {
	#		chr1    100341395       100341396       AT      A       1       5       6       1
			chomp $l;
			my @a = split (/\t/,$l);
			my $bases=$a[5];
			my $reference=$a[3];
			...
			my $uniq = $a[0]."_".$a[1]."_".$a[2]."_".$a[8]."_".$reference."_".$alternate."_".$bases;
			if ($list =~ / $uniq /)	{
				$indel{$uniq} .= "$a[6]\t$a[7]\t";		
			}
			else	{
				$indel{$uniq} = "$a[0]\t$a[1]\t$a[2]\t$a[8]\t$reference\t$alternate\t$bases\t";  ## Common chr start stop Bases	
			    for (my $j=0; $j<$i; $j++)	{
					$indel{$uniq}.="n/a\tn/a\t";
	    		}
				$indel{$uniq}.= "$a[6]\t$a[7]\t";
