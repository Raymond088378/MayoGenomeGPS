...
"\n<Resource name=\"$sam1[$j].igv-sorted.bam\" path=\"http://$server/secondary/$PI/$folder/IGV_BAM/$groupNames[$i]/$sampleNames[$j].igv-sorted.bam\" />";
		...
		if ( $analysis eq "mayo" || $analysis eq "realign-mayo" ) 
		{
			...
			$delivery_folder =~ s/\/data2/ftp:\/\/rcfisinl1-212/g;
			print FH
"\n<Resource name=\"$groupNames[$i].variants.filter.final.vcf\" path=\"$delivery_folder/Reports_per_sample/$groupNames[$i].variants.final.vcf\" />";
			if ($somatic eq "YES")
			{
				print FH
"\n<Resource name=\"$groupNames[$i].somatic.variants.filter.final.vcf\" path=\"$delivery_folder/Reports_per_sample/$groupNames[$i].somatic.variants.final.vcf\" />";
			}
		...
		{
			print FH
"\n<Resource name=\"$groupNames[$i].variants.filter.final.vcf\" path=\"http://$server/secondary/$PI/$folder/Reports_per_sample/$groupNames[$i].variants.final.vcf\" />";
		if ($somatic eq "YES")
			{
				print FH
"\n<Resource name=\"$groupNames[$i].somatic.variants.filter.final.vcf\" path=\"http://$server/secondary/$PI/$folder/Reports_per_sample/$groupNames[$i].somatic.variants.final.vcf\" />";
			}
		}
...
"\n<Resource name=\"$sampleNames[$i].igv-sorted.bam\" path=\"$delivery_folder/IGV_BAM/$sampleNames[$i].igv-sorted.bam\" />";
			print FH 
"\n<Resource name=\"$sampleNames[$i].variants.filter.final.vcf\" path=\"$delivery_folder/Reports_per_sample/$sampleNames[$i].variants.final.vcf\" />";
		}
		else {
			print FH
"\n<Resource name=\"$sampleNames[$i].igv-sorted.bam\" path=\"http://$server/secondary/$PI/$folder/IGV_BAM/$sampleNames[$i].igv-sorted.bam\" />";
			print FH 
"\n<Resource name=\"$sampleNames[$i].variants.filter.final.vcf\" path=\"http://$server/secondary/$PI/$folder/Reports_per_sample/$sampleNames[$i].variants.final.vcf\" />";
		}
	}
} ## Single Samples 
