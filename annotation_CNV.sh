	...
	echo "Multi sample"
        ...
        do
                ...
                do
                        ...
                        ### preparing segseq files
                        segseq=$CNV_dir/$group
                        touch $segseq/$sample.del.tmp $segseq/$sample.dup.tmp $segseq/$sample.del.bed $segseq/$sample.dup.bed 

                        ### merging per chromosome file for del and dup
                        for j in `seq 1 22` X Y M
                        do
                                cat $segseq/$sample.$j.del.bed >> $segseq/$sample.del.tmp
                                cat $segseq/$sample.$j.dup.bed >> $segseq/$sample.dup.tmp
                        done
                
                        cat $segseq/$sample.del.tmp | awk '{print $1"\t"$2"\t"$3"\t""segseq_DEL""\t"$4"\t"$5"\t"$6}' > $segseq/$sample.del.bed
                        cat $segseq/$sample.dup.tmp | awk '{print $1"\t"$2"\t"$3"\t""segseq_DUP""\t"$4"\t"$5"\t"$6}' > $segseq/$sample.dup.bed
                        
                        ### concatenating cnvnator and segseq files
                        touch $segseq/$sample.segseq.bed
                        cat $segseq/$sample.del.bed $segseq/$sample.dup.bed >> $segseq/$sample.segseq.bed
                        
                        $script_path/segseq_tumor-normal.sh $output_dir $run_info
                        
                        rm $segseq/$sample.del.tmp $segseq/$sample.dup.tmp $segseq/$sample.del.bed $segseq/$sample.dup.bed 
                done
        done
        ...
        do
                ...
                do
                        ...
                        then
                                ...
                                $bedtools/intersectBed -b $segseq/$sample.segseq.bed.tumor.specific -a $master_gene_file -wb > $segseq/$sample.segseq.intersect.bed
                                cat $segseq/$sample.segseq.intersect.bed | awk '{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$7"\t"$8"\t"$11"\t"$9}' > $segseq/$sample.segseq.intersect.annotated.bed
                                $bedtools/sortBed -i $segseq/$sample.segseq.intersect.annotated.bed > $segseq/$sample.segseq.intersect.annotated.sorted.bed
                        
                                ###  generating per sample CNV annotation files
                                touch $report_dir/$sample.CNV.annotated.txt
                                echo -e "Chr\tGene_Start\tGene_Stop\tGene\tStrand\tCNV_Start\tCNV_Stop\tCNV_Count\tCNV_Type" >> $report_dir/$sample.CNV.annotated.txt
                                cat $segseq/$sample.segseq.intersect.annotated.sorted.bed >> $report_dir/$sample.CNV.annotated.txt
                        
                                ### removing intermediate files
                                rm $segseq/$sample.segseq.intersect.bed $segseq/$sample.segseq.intersect.annotated.bed $segseq/$sample.segseq.intersect.annotated.sorted.bed 
                        else
