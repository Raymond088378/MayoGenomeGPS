...
then
	...
	##intersect with two bed files
	#blacklisted=wgEncodeDacMapabilityConsensusExcludable.bed
	#mapability=wgEncodeDukeMapabilityRegionsExcludable.bed
	...
	perl $script_path/MergeListReport_SeattleSeq.pl -i $file.sift.codons.map.repeat.base.snp.UCSCtracks -s $sseq/sseq.snvs.out.allsamples.chr${which_chr}.merge -c $chr -p $pos -r $ref -a $alt -o $TempReports/$var.SNV.report
