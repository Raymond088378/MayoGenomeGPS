...
then
	...
	then
		...
		then
			...
			$script_path/CREST2VCF.pl -i $output_dir/$group/${sample}.$chr.predSV.txt -f $ref_genome -o $output_dir/$group/${sample}.$chr.vcf -s $sample -t $samtools
			if [ ! -s $output_dir/$group/${sample}.$chr.vcf.fail ]
			then
				rm $output_dir/$group/${sample}.$chr.vcf.fail
			fi  
			$script_path/vcfsort.pl ${ref_genome}.fai $output_dir/$group/${sample}.$chr.vcf > $output_dir/$group/${sample}.$chr.vcf.sort
			mv $output_dir/$group/${sample}.$chr.vcf.sort $output_dir/$group/${sample}.$chr.vcf
			awk "((\$10>=$min_read)&&(\$11>=$min_read)&&(\$14>=$min_id)&&(\$16>=$min_id))" $output_dir/$group/${sample}.$chr.predSV.txt | awk '{print $1"\t"$2"\t"$2+1"\t"$5"\t"$6"\t"$6+1}' | $bedtools/pairToBed -a stdin -b $blacklist_sv -type neither | $script_path/report_original.pl $output_dir/$group/${sample}.$chr.predSV.txt > $output_dir/$group/${sample}.$chr.final.predSV.txt
			$script_path/CREST2VCF.pl -i $output_dir/$group/${sample}.$chr.final.predSV.txt -f $ref_genome -o $output_dir/$group/${sample}.$chr.final.vcf -s $sample -t $samtools
			if [ ! -s $output_dir/$group/${sample}.$chr.final.vcf.fail ]
			then
				rm $output_dir/$group/${sample}.$chr.final.vcf.fail
			fi  
			$script_path/vcfsort.pl ${ref_genome}.fai $output_dir/$group/${sample}.$chr.final.vcf > $output_dir/$group/${sample}.$chr.final.vcf.sort
			mv $output_dir/$group/${sample}.$chr.final.vcf.sort $output_dir/$group/${sample}.$chr.final.vcf
			if [ -f $output_dir/$group/${sample}.$chr.predSV.txt ]
			then
			...
			$script_path/CREST2VCF.pl -i $output_dir/$group/${sample}.$chr.predSV.txt -f $ref_genome -o $output_dir/$group/${sample}.$chr.vcf -s $sample -t $samtools
			rm $output_dir/$group/${sample}.$chr.vcf.fail
			touch  $output_dir/$group/${sample}.$chr.final.predSV.txt
			$script_path/CREST2VCF.pl -i $output_dir/$group/${sample}.$chr.final.predSV.txt -f $ref_genome -o $output_dir/$group/${sample}.$chr.final.vcf -s $sample -t $samtools
			rm $output_dir/$group/${sample}.$chr.final.vcf.fail
		fi
		`kill -9 $pid`	
